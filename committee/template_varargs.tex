...
Variable-length template parameter lists allow a class or function
template to accept some number (possibly zero) of template arguments
beyond the number of template parameters specified. This behavior can
be simulated in C++ via a long list of defaulted template parameters,
e.g., a typelist wrapper may appear as:
...
users aren't likely to use this feature, as any use of a heterogenous
type container ({\tt tuple}, in this case) requires some degree of
metaprogramming, although advanced users may require this
functionality. 

\subsection{Typesafe Variable-length Function Parameter Lists}
Variable-length function parameter lists (denoted by an ellipsis at
the end of a function parameter list) allow an arbitrary number of
function call arguments to be passed to a function that declares fewer
parameters than it is passed. This feature is rarely used in C++ code
(except for compatibility with C libraries), because passing non-POD
types through {\tt ...} invokes undefined behavior. However, such a
feature is useful in some contexts, e.g., for implementing a type-safe
C++ {\tt printf} that works for non-POD types. The lack of such a
facility has resulted in odd syntax for formatting~\cite{Krempp02},
e.g.:
\begin{verbatim}
...
This proposal solves both issues: arguments are passed via the ``Const
+ Non-const reference'' option \#3 enumerated in~\cite{Dimov02}, which
results in ``perfect'' forwarding of arguments, but does not require
$2^N$ overloads to support $N$ arguments: instead, a single overload
suffices for any number of arguments. The solution matches all three
criteria (C1-C3) of the argument forwarding paper~\cite{Dimov02}, but
bundling the arguments into a single argument and provides an
...
Nontype and template template arguments are not placed directly in
...
itself not a template. The {\tt apply} member class template accepts
some number of template arguments and instantiates the template
designated by the {\tt Designator} with those parameters. Thus
instantiating {\tt Foo<int, std::vector>} results in {\tt Args} being
{\tt std::tuple<template\_template\_arg<X> >} (where {\tt X} is
unspecied and unique to {\tt std::vector}) and where {\tt
  template\_template\_arg<X>::apply<int>::type} is equivalent to {\tt
  std::vector<int>}. 

Default template arguments will be applied when fewer template
arguments are specified in {\tt Args} than are defined for {\tt U}.
Thus, the instantiation {\tt Foo<int, std::vector, 17>} would result
in Args being the tuple \\ {\tt std::tuple<template\_template\_arg<???>,
  nontype\_template\_arg<int, 17>}.
...
this case (and barring an ellipsis in the function parameter list, as
described in the next section) arguments may only by specified
explicitly when calling the function template; they are not deduced,
e.g.,:
...
\subsection{Typesafe variable-length function parameter lists}
When a function template contains an ellipsis (followed by an
...
the-return-type apply(F Class::*pm, Class& obj, tuple<T1, T2, ..., TN>& t);

template<typename F, typename Class, typename T1, typename T2, ..., typename TN>
the-return-type apply(F Class::*pm, const Class& obj, tuple<T1, T2, ..., TN>& t);
...
With this definition, we can define a function {\tt g} that forwards
...
support partial ordering with template varargs, introduce an
additional bullet:
\begin{itemize}
\item If after removing the variable-length template parameter list
  ...
  parameters to the shorted template parameter list until the template
...
\section{Examples}
This section gives skeletal implementations of four library
components, including Tuples, all of which were accepted by the
library working group into the first library TR. The implementations
are not complete, but omit only features that are irrelevant to the
discussion of template varargs. 
