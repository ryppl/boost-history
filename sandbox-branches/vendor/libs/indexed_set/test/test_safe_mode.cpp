...
#define CATCH_SAFE_MODE(err) \
  ...
  if(e.error_code!=(err))throw std::runtime_error("safe mode violation not expected");\
}
...
void test_safe_mode()
{
  ...
  /* this, unlike the previous case, is indeed correct, test safe mode gets it right */
  { 
    employee_set es3(es);
    employee_set es4;
    employee_set::iterator it=es3.begin();
