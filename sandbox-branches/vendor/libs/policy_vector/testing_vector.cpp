...
// Testing sizes
#ifndef POLICY_VECTOR_DEFAULT_SIZE
#warning "No default size selected, picked 100,000; change by compiling with\n"\
         "-DPOLICY_VECTOR_DEFAULT_SIZE=N, where N is a number that makes the\n"\
	 "timings meaningful yet manageable"
const std::size_t N = 100000;  // actual size of container
#else
const std::size_t N = POLICY_VECTOR_DEFAULT_SIZE;  // actual size of container
#endif
...
#ifndef POLICY_VECTOR_TESTING_CUSTOM
...
void test_array(const Array &a, bool checking, bool iter_checking)
{
  ...
  test_type<typename Array::pointer>(); 
  test_type<typename Array::const_pointer>(); 
...
{
#if !defined(POLICY_VECTOR_TESTING_FIXED_EXTERNAL_RANGE)
  // create and initialize an array of same size
  float data[Array::static_size];
  for (std::size_t i=0; i<Array::static_size; ++i) data[i] = i;    
  const Array a( data, typename Array::allocator_type(), sp );
#else
  // Exceptionally, can't do that, since external ranges don't have
  // static size...
  float data[N];
...
void test_variable_size()
{
  ...
  test_variable_size_policy<StoragePolicy,
    no_check, external_iterator_adaptor>(false, false);
  std::cout << "    b. CHECKED ITERATOR ADAPTOR, EXCEPTION CHECKING\n";
  test_variable_size_policy<StoragePolicy,
    exception_check, checked_external_iterator_adaptor>(true, true);
  std::cout << "    c. CHECKED ITERATOR ADAPTOR, ASSERT CHECKING\n";
  test_variable_size_policy<StoragePolicy,
    assert_check, checked_external_iterator_adaptor>(false, false);
  // for assert, don't check, otherwise assert will stop the program
#endif
}
...
#if defined(POLICY_VECTOR_TESTING_FIXED_EXTERNAL_STATIC_ARRAY)
  separator();
  elem_t data[N];
  std::cout <<
    "2.1 TESTING FIXED EXTERNAL STATIC ARRAY\n";
  test_fixed_size< external >( external(data) );
  separator();
#endif
