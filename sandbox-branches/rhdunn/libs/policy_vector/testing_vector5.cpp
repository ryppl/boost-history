...
void test_array(const Array &a, bool checking)
{
  // test typedefs
...
void test_fixed_size_array(bool checking)
{
  // define special type names
  typedef policy_vector<float, StoragePolicy, CheckingPolicy> Array;
...
void test_fixed_size()
{
  ...
  test_fixed_size_array<StoragePolicy, no_check>(false);
  std::cout << "    c. RAW POINTER ITERATOR, EXCEPTION CHECKING\n";
  test_fixed_size_array<StoragePolicy, exception_check>(true);
...
template <class StoragePolicy,
	  template <class S> class CheckingPolicy>
void test_variable_size_array(bool checking)
{
  // define special type names and temp variable
  typedef policy_vector<float, StoragePolicy, CheckingPolicy> Array;
  typedef typename Array::const_iterator const_iterator;
  float tmp[4] = { -0.8, -0.6, -0.4, -0.2 };
  ...
  if (a[0] != -1.0 || a[1] != -1.0 || a[2] != -1.0
   || a[3] != tmp[0] || a[4] != tmp[1] || a[5] != tmp[2] || a[6] != tmp[3])
    ...
    pos = a.begin()+7;
    for (std::size_t i=0; pos != a.end(); ++pos, ++i)
      if (*pos != i) {
        std::cout << "\terase position is BROKEN\n";
        break;
      }
...
{
  std::cout << "    a. RAW POINTER ITERATOR, NO CHECKING\n";
  test_variable_size_array<StoragePolicy, no_check>(false);
  std::cout << "    b. RAW POINTER ITERATOR, EXCEPTION CHECKING\n";
  test_variable_size_array<StoragePolicy, exception_check>(true);
}
