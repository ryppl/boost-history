...
void is_empty_test()
{
    ...
    shared_ptr<ifirst> ptr2(new cfirst<1>);
    BOOST_CHECK(ptr2 ? true : false);
    BOOST_CHECK(!!ptr2);
    BOOST_CHECK(ptr2.get() != 0);

    shared_ptr<ifirst> ptr3(shared_ptr<ifirst>(new cfirst<1>));
...
{
    test_suite* test = BOOST_TEST_SUITE("shared_ptr is empty test");
