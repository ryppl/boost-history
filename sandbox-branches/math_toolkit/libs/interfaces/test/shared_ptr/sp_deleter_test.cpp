...
{
    {
        shared_ptr<ifirst> ptr(new cfirst<1>, destroyer< cfirst<1> > ());
    }
    BOOST_CHECK(counter< cfirst<1> >::value() == 0);

    {
        shared_ptr<ifirst> ptr(new cderived1<1>, destroyer< cderived1<1> >());
    }
    BOOST_CHECK(counter< cderived1<1> >::value() == 0);

    {
        shared_ptr<ifirst> ptr(new cderived4<1>, destroyer< cderived4<1> >());
    }
...
{
    test_suite* test = BOOST_TEST_SUITE("shared_ptr deleter test");
