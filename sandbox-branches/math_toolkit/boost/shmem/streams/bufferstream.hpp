...
#ifndef BOOST_SHMEM_BUFFERSTREAM_HPP
#define BOOST_SHMEM_BUFFERSTREAM_HPP
   ...
   : public std::basic_streambuf<CharT, CharTraits>
{
      ...
      if(m_mode & std::ios_base::out) {
         if(!CharTraits::eq_int_type(c, CharTraits::eof())) {
            if(!(m_mode & std::ios_base::in)) {
               ...
               // We're not using a special append buffer, just the buffer itself.
               if(this->pptr() == this->epptr()) {
   ...
   virtual std::streamsize xsputn(const char_type* s, std::streamsize n)
   {
      ...
      if((m_mode & std::ios_base::out) && n > 0) {
         ...
         // buffer, then overwrite instead of append.
         assert(this->pbase() == m_buffer);
         std::streamsize avail = static_cast<std::streamsize>
            (m_buffer + m_length - this->pptr());
         if(avail > n) {
...
#endif /* BOOST_SHMEM_BUFFERSTREAM_HPP */
