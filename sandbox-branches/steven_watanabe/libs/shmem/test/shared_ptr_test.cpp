...
#include <boost/shmem/smart_ptr/enable_shared_from_this.hpp>
...
int simple_test()
{
   typedef allocator<base_class, named_shared_object::segment_manager>
      base_class_allocator;
   typedef shared_ptr_deleter<named_shared_object::segment_manager>
      shared_ptr_deleter_t;
   typedef shared_ptr<base_class, base_class_allocator, shared_ptr_deleter_t> my_shared_ptr;
   typedef weak_ptr<base_class, base_class_allocator, shared_ptr_deleter_t> my_weak_ptr;

   named_shared_object shmem;

   shmem.create("shm_name", 10000);

   {
      my_shared_ptr s_ptr(my_shared_ptr::pointer(0), 
                        base_class_allocator(shmem.get_segment_manager()), 
                        shared_ptr_deleter_t(shmem.get_segment_manager()));

      my_shared_ptr s_ptr2(shmem.construct<base_class>("base_class")(), 
                           base_class_allocator(shmem.get_segment_manager()), 
                           shared_ptr_deleter_t(shmem.get_segment_manager()));

      my_shared_ptr s_ptr3(offset_ptr<derived_class>(shmem.construct<derived_class>("derived_class")()), 
                           base_class_allocator(shmem.get_segment_manager()), 
                           shared_ptr_deleter_t(shmem.get_segment_manager()));
...
int string_shared_ptr_vector_insertion_test()
{
   ...
   typedef allocator<string_t, named_shared_object::segment_manager>
      string_allocator_t;

   //A deleter for shared_ptr<> that erases a shared memory string
   typedef shared_ptr_deleter<named_shared_object::segment_manager> deleter_t;

   //A shared pointer that points to a shared memory string and its instantiation
   typedef shared_ptr<string_t, string_allocator_t, deleter_t> string_shared_ptr_t;

   //An allocator for shared pointers to a string in shared memory 
   typedef allocator<string_shared_ptr_t, named_shared_object::segment_manager >
      string_shared_ptr_allocator_t;

   //A weak pointer that points to a shared memory string and its instantiation
   typedef weak_ptr<string_t, string_allocator_t, deleter_t> string_weak_ptr_t;

   //An allocator for weak pointers to a string in shared memory 
   typedef allocator<string_weak_ptr_t, named_shared_object::segment_manager >
   ...
   {  
      ...
      //This is a string deleter using destroy_ptr() function of the named_shared_object
      deleter_t deleter(shmem.get_segment_manager());
      ...
      scoped_ptr<string_t, deleter_t> scoped_string
      ...
      for(;beg != end; ++beg){
         ...
         catch(const boost::shmem::bad_weak_ptr &){
            success = true;
         }
         if(!success){
            return -1;
...
template<class T, class U, class A, class D>
void test_shared(weak_ptr<T, A, D> const & a, weak_ptr<U, A, D> const & b)
{
   BOOST_TEST(!(a < b));
   BOOST_TEST(!(b < a));
}
...
template<class T, class U, class A, class D>
void test_nonshared(weak_ptr<T, A, D> const & a, weak_ptr<U, A, D> const & b)
{
   BOOST_TEST(a < b || b < a);
   BOOST_TEST(!(a < b && b < a));
}
...
{
   typedef allocator<void, named_shared_object::segment_manager>
      v_allocator_t;

   typedef shared_ptr_deleter<named_shared_object::segment_manager>
      deleter_t;

   typedef shared_ptr<X, v_allocator_t, deleter_t> x_shared_ptr;

   typedef shared_ptr<Y, v_allocator_t, deleter_t> y_shared_ptr;

   typedef weak_ptr<X, v_allocator_t, deleter_t> x_weak_ptr;

   typedef weak_ptr<Y, v_allocator_t, deleter_t> y_weak_ptr;
   ...
   shmem.create("shm_name", 10000);
   {
      ...
      x_shared_ptr p (shmem.construct<Y>(anonymous_instance)(), v_allocator, deleter);
      x_shared_ptr p2(shmem.construct<X>(anonymous_instance)(), v_allocator, deleter);
      ...
      y_shared_ptr p4 (dynamic_pointer_cast<Y>(p2));
      test_is_nonzero(p3);
      test_is_zero(p4);
      ...
      shared_ptr<void, v_allocator_t, deleter_t> p5(p);
      ...
      catch(boost::shmem::bad_weak_ptr const &)
      {}

      test_is_zero(wp1.lock());

      x_weak_ptr wp2 = static_pointer_cast<X>(p5);
