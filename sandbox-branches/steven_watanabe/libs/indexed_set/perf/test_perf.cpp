/* Boost.IndexedSet performance test.
 *
...
/* node size of a given indexed_set instantiation */

template <typename IndexedSet>
size_t indexed_set_node_size(const IndexedSet&)
{
  return sizeof(*IndexedSet().begin().get_node());
}
...
/* mono_container runs the tested routine on indexed_sets and manual
...
struct mono_container
{
  ...
  static size_t indexed_set_node_size()
  {
    return ::indexed_set_node_size(Container());
  }
...
struct tri_container
{
  ...
  static size_t node_size()
  {
    return ::node_size(Container1())+::node_size(Container2())+::node_size(Container3());
  }
  ...
  BOOST_APPEND_EXPLICIT_TEMPLATE_TYPE(ManualTest))
{
  ...
  size_t indexed_t_node_size=IndexedTest::indexed_set_node_size();
...
/* compare_structures* accept an indexed_set instantiation and
...
template <typename IndexedType,typename ManualType1,typename ManualType2,typename ManualType3>
...
{
  {
    ...
    typedef indexed_set<int>         indexed_t;
  ...
  }
  {
    ...
    typedef indexed_set<
      int,
      index_list<
        boost::indexed_sets::unique<identity<int> >,
