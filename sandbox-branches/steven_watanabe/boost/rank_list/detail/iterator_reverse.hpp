//  boost/rank_list/detail/iterator_reverse.hpp
...
/*
  ...
  Reverse rank_list iterators (both mutable and const
...
#ifndef _BOOST_RANK_LIST_ITERATOR_REVERSE_HPP_
#define _BOOST_RANK_LIST_ITERATOR_REVERSE_HPP_
...
class rank_list_rev_iter               // const_iterator
{
  friend class boost::rank_lists::rank_list<T,A,W,P>;

  typedef rank_list_node_tree_fields<T,A,W,P>    node_t;
  typedef rank_list_iterator<T,A,W,P,Ref,Ptr>    my_reverse;
  typedef rank_list_rev_iter<T,A,W,P,Ref,Ptr>    my_class;
  ...
  public: // -------------- PUBLIC INTERFACE ----------------
    ...
    typedef typename
            my_rlist::const_reverse_iterator     const_iterator;

    // Constructors: all O(1)

    rank_list_rev_iter ();                    // Singular iterator
    rank_list_rev_iter (const my_class & it); // Copy constructor
    explicit
    rank_list_rev_iter (const my_reverse & it); // From reverse
    ...
    template<class X,class Y> difference_type operator-
      (const rank_list_rev_iter<T,A,W,P,X,Y> & it) const;
  ...
  private: // ----- PRIVATE DATA MEMBER AND HELPER FUN. ------
    ...
    rank_list_rev_iter (node_t * node)  // Only rank_list has
...
inline rank_list_rev_iter<T,A,W,P,Ref,Ptr>::
  rank_list_rev_iter () : ptr(NULL) {}

// Copy constructor: just copy the embedded pointer

template<class T,class A,class W,class P,class Ref,class Ptr>
inline rank_list_rev_iter<T,A,W,P,Ref,Ptr>::
  rank_list_rev_iter (const my_class & it) { ptr = it.ptr; }
...
template<class T,class A,class W,class P,class Ref,class Ptr>
inline rank_list_rev_iter<T,A,W,P,Ref,Ptr>::
  rank_list_rev_iter (const my_reverse & it) { ptr = it_ptr(it); }

// Conversion to const iterator. Again through the rank_list class

template<class T,class A,class W,class P,class Ref,class Ptr>
inline rank_list_rev_iter<T,A,W,P,Ref,Ptr>::
  operator typename rank_list_rev_iter<T,A,W,P,Ref,Ptr>::
  const_iterator ()
{ return my_rlist::make_const_rev_iter(ptr); } // (reverse...)

// Dereference. data() asserts that this is neither a singular
// iterator nor an end node.

template<class T,class A,class W,class P,class Ref,class Ptr>
inline typename rank_list_rev_iter<T,A,W,P,Ref,Ptr>::reference
  rank_list_rev_iter<T,A,W,P,Ref,Ptr>::operator* () const
{ return my_rlist::data (ptr); }

// The arrow can be used when T is a struct or class

template<class T,class A,class W,class P,class Ref,class Ptr>
inline typename rank_list_rev_iter<T,A,W,P,Ref,Ptr>::pointer
  rank_list_rev_iter<T,A,W,P,Ref,Ptr>::operator->() const
{ return &**this; }
...
template<class T,class A,class W,class P,class Ref,class Ptr>
inline typename rank_list_rev_iter<T,A,W,P,Ref,Ptr>::reference
  rank_list_rev_iter<T,A,W,P,Ref,Ptr>::operator[]
  (typename rank_list_rev_iter<T,A,W,P,Ref,Ptr>::difference_type n)
                                                              const
{ return *(*this+n); }
...
template<class T,class A,class W,class P,class Ref,class Ptr>
inline
  typename rank_list_rev_iter<T,A,W,P,Ref,Ptr>::const_reference
  rank_list_rev_iter<T,A,W,P,Ref,Ptr>::operator()
  (typename rank_list_rev_iter<T,A,W,P,Ref,Ptr>::difference_type n)
                                                              const
{ return *(*this+n); }
...
template<class T,class A,class W,class P,class Ref,class Ptr>
inline rank_list_rev_iter<T,A,W,P,Ref,Ptr> &
  rank_list_rev_iter<T,A,W,P,Ref,Ptr>::operator++ ()  // (pre++)
{
  ptr = my_rlist::prev (ptr);    // Step back! (reverse...)
  return *this;
}
...
inline rank_list_rev_iter<T,A,W,P,Ref,Ptr> &
  rank_list_rev_iter<T,A,W,P,Ref,Ptr>::operator-- ()  // (pre--)
{
  ptr = my_rlist::next (ptr);    // Step forward! (reverse...)
  return *this;
}

template<class T,class A,class W,class P,class Ref,class Ptr>
inline rank_list_rev_iter<T,A,W,P,Ref,Ptr>
  rank_list_rev_iter<T,A,W,P,Ref,Ptr>::operator++ (int) // (post++)
{
  my_class tmp(*this);
  ptr = my_rlist::prev (ptr);    // Step back! (reverse...)
...
{
  my_class tmp(*this);
  tmp.ptr = my_rlist::jump (tmp.ptr, -n, false);   // Note: -n
  RL_ASSERT_EXC (tmp.ptr, index_out_of_bounds());  // (reverse...)
  return tmp;
}                           // jump() takes logarithmic time

template<class T,class A,class W,class P,class Ref,class Ptr>
inline rank_list_rev_iter<T,A,W,P,Ref,Ptr> operator+
  (typename rank_list<T,A,W,P>::difference_type n,
   const rank_list_rev_iter<T,A,W,P,Ref,Ptr> & it)
{ return it + n; }
...
inline
  typename rank_list_rev_iter<T,A,W,P,Ref,Ptr>::difference_type
  rank_list_rev_iter<T,A,W,P,Ref,Ptr>::operator-
  (const rank_list_rev_iter<T,A,W,P,X,Y> & it)    const
{
  my_rlist * a, * b;
...
template<class T, class A, class W, class P,
         class Ref, class Ptr>
inline
  typename rank_list_rev_iter<T,A,W,P,Ref,Ptr>::iterator_category
  iterator_category (const rank_list_rev_iter<T,A,W,P,Ref,Ptr>&)
{
  return typename rank_list_rev_iter<T,A,W,P,Ref,Ptr>::
                                          iterator_category();
}
...
template<class T, class A, class W, class P,
         class Ref, class Ptr>
inline
  typename rank_list_rev_iter<T,A,W,P,Ref,Ptr>::value_type *
  value_type (const rank_list_rev_iter<T,A,W,P,Ref,Ptr>&)
{
  return reinterpret_cast<
          typename rank_list_rev_iter<T,A,W,P,Ref,Ptr>::
                                             value_type *>(0);
}
...
inline
  typename rank_list_rev_iter<T,A,W,P,Ref,Ptr>::difference_type *
  distance_type (const rank_list_rev_iter<T,A,W,P,Ref,Ptr>&)
{
  return reinterpret_cast<
          typename rank_list_rev_iter<T,A,W,P,Ref,Ptr>::
                                        difference_type *>(0);
