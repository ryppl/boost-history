//  boost/rank_list/detail/rl_insert.hpp
...
#ifndef _BOOST_RANK_LIST_INSERT_HPP_
#define _BOOST_RANK_LIST_INSERT_HPP_
...
// it must be clarified that constructing a new rlist with
...
inline
  typename rank_list<T,A,W,P>::iterator     // Insert anywhere
  rank_list<T,A,W,P>::insert
  (typename rank_list<T,A,W,P>::const_reference t) // Original
{
  node_t * newnode;
...
inline
  ...
  rank_list<T,A,W,P>::insert
  (const typename rank_list<T,A,W,P>::iterator & it, // Where
   typename rank_list<T,A,W,P>::const_reference t)   // Original
{
  node_t * newnode;
...
inline
  ...
  (const typename                                   // Where and
     rank_list<T,A,W,P>::reverse_iterator & it,     // how (REV.)
   typename rank_list<T,A,W,P>::const_reference t)  // Original
{
  node_t * newnode;
...
//not inline
  ...
  rank_list<T,A,W,P>::insert
  (const typename rank_list<T,A,W,P>::iterator it,  // Where
   typename rank_list<T,A,W,P>::size_type n,        // How many
   typename rank_list<T,A,W,P>::const_reference t)  // Original
{
  ...
  RL_ASSERT_HO (owner(p)==this);  // it must point into this arr.
...
inline
  ...
  (const typename rank_list<T,A,W,P>::iterator & it, // Where
   ...
   typename rank_list<T,A,W,P>::const_reference t)   // Original
{
  RL_ASSERT (n>=0);  // Can't insert a negative amount
   ...
   typename rank_list<T,A,W,P>::const_reference t)  // Original
{
  ...
  RL_ASSERT_HO (owner(it.ptr)==this); // it must point
                                      // into this arr.
  if (n==0) return;
...
//not inline
  void
  rank_list<T,A,W,P>::insert
  (typename rank_list<T,A,W,P>::iterator it,    // Where
   IT from,
   IT to)                // Originals (*to not included)
{
  ...
  size_type n;
                                      // it must point
  RL_ASSERT_HO (owner(it.ptr)==this); // into this arr.
...
//not inline
  void
  rank_list<T,A,W,P>::insert
  (typename rank_list<T,A,W,P>::reverse_iterator it, // Where
   IT from,
   IT to)                     // Originals (*to not included)
{
#ifdef BOOST_CLASS_REQUIRE
...
//not inline
  void
  rank_list<T,A,W,P>::insert_before
  (typename rank_list<T,A,W,P>::node_t * newnode,
   typename rank_list<T,A,W,P>::node_t * p)
{
  ...
  RL_ASSERT (newnode);   // Can't insert NULL
...
// constructing a new rlist with N elements from scratch
...
//not inline
  void
  rank_list<T,A,W,P>::insert_anywhere
  (typename rank_list<T,A,W,P>::node_t * newnode)
{
