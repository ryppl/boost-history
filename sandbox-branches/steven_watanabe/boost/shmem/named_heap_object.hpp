...
#ifndef BOOST_SHMEM_NAMED_HEAP_OBJECT_HPP
#define BOOST_SHMEM_NAMED_HEAP_OBJECT_HPP
   ...
   : public detail::basic_named_object_impl <CharType, AllocationAlgorithm, IndexType>
{
 ...
 public: //functions
   ...
   {
      if(!resize_vector(size) ||
         !base_t::create_impl(&m_heapmem[0], size)){
         close();
         return false;
      }
      return true;
 ...
 private:
   ...
   bool resize_vector(std::size_t size)
   {
      BOOST_TRY{
         m_heapmem.resize(size, char(0));
         return true;
...
#endif   //BOOST_SHMEM_NAMED_HEAP_OBJECT_HPP
