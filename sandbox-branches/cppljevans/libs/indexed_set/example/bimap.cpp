/* Boost.IndexedSet example of a bidirectional map.
 *
 ...
 * of a bidirectional map based on indexed_set.
 */
...
/* see Advanced topics: Use of member_offset for info on member<> and member_offset<> */
  ...
  typedef indexed_set<
    value_type,
    index_list<
      unique<tag<from>,member_offset<value_type,FromType,from_offset> >,
      unique<tag<to>,  member_offset<value_type,ToType,to_offset> >
    >
  > type;

#else

  /* A bidirectional map can be simulated as an indexed_set
  ...
  typedef indexed_set<
    value_type,
    index_list<
      unique<tag<from>,member<value_type,FromType,&value_type::first> >,
      unique<tag<to>,  member<value_type,ToType,&value_type::second> >
    >
  > type;
...
int main()
{
  ...
  if(it!=d.end()){ /* found */
    ...
    dictionary::iterator_type<to>::type it2=d.get<to>().find(word);
    if(it2!=d.get<to>().end()){
      std::cout<<word<<" is said "<<it2->first<<" in Spanish"<<std::endl;
    }
    else std::cout<<"No such word in the dictionary"<<std::endl;
