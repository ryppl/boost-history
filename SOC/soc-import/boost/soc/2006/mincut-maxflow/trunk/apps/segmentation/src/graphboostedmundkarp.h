...
#ifndef GRAPHBOOSTEDMUNDKARP_H
#define GRAPHBOOSTEDMUNDKARP_H
...
*/
class GraphBoostEdmundKarp : public GraphBase
{
...
public:
	GraphBoostEdmundKarp(unsigned int f_numberOfVertices)
		:m_graph(f_numberOfVertices+2),m_capacity(get(edge_capacity, m_graph)),m_reverse_edge(get(edge_reverse,m_graph)),
   m_source_vertex(vertex(f_numberOfVertices,m_graph)),m_sink_vertex(vertex(f_numberOfVertices+1,m_graph)),m_segment(f_numberOfVertices,GraphBase::SINK),m_segments_calculated(false){ //add source and terminal as last 2 verts
		}

		~GraphBoostEdmundKarp(){
		};
		...
		virtual tPrecision maxflow(){  
			GraphBase::tPrecision flow=edmunds_karp_max_flow(m_graph, m_source_vertex, m_sink_vertex);
			return flow;
		}
