...
void FreeGraph::reset()
{
  ...
  A.assign( nnodes,false);
  
  S.assign( nnodes,false);
...
bool FreeGraph::error()
{
  return berror;
}

FreeGraph::termtype FreeGraph::what_segment(int k) const
{
  // the segment that the node is in is just it's membership in S or
  // not.
  return (S[k] ? SOURCE : SINK);
}

void FreeGraph::add_arc(int from, int to, ncaptype cap)
{
...
void FreeGraph::add_edge(int from, int to, ncaptype cap, ncaptype cap_rev)
{
...
void FreeGraph::add_weight(int k, termtype terminal, tcaptype delta)
{
  ...
  if (terminal == SOURCE){
    ...
    if ((pnode->cap_left[terminal] > 0) && (A[node_id(pnode)] == 0)){
       A[node_id(pnode)]=true;
       S[node_id(pnode)]=true;

//       A.bit_set(node_id(pnode));DD
//       S.bit_set(node_id(pnode));DD
	...
	S[node_id(pnode)]=false;
	A[node_id(pnode)]=false;
...
FreeGraph::node_t * FreeGraph::grow()
{
  ...
  while (Alist != NULL){
    ...
    for (int i = 0 ; i < p->nedges ; i++){
      ...
      if ((S[node_id(q)] == 0) && (edge_cap(pedge, p) > 0)){
        // S = S U {q}
        S[node_id(q)]=true;
...
void FreeGraph::augment(FreeGraph::node_t * psink)
{
...
void FreeGraph::process(FreeGraph::node_t * p)
{
