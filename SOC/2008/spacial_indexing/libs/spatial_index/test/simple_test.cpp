//
// Boost.SpatialIndex - basic test for quadtree
//
...
int test_main(int, char* [])
{
  ...
  boost::spatial_index::spatial_index<point_type, value_type, 
        boost::spatial_index::quadtree<point_type, value_type> > q(b, 1);
  ...
  unsigned int i = 0;
  for(std::deque< std::vector<std::string>::iterator >::const_iterator dit = 
      d.begin(); dit != d.end(); ++dit) {
