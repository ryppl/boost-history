...
void const_fixed_interface()
{
    ...
    fixed_view< const_view<ifirst> > first = first1;
    first.one("");
    first.two();
    ...
    fixed_view< const_view<iderived1> > derived1 = derived1_1;
    derived1.one("");
    derived1.two();
    derived1.three("");
    derived1.four();
    ...
    fixed_view< const_view<iderived4> > derived4 = derived4_1;
    derived4.one("");
    derived4.two();
    derived4.three("");
    derived4.four();
...
{
    test_suite* test = BOOST_TEST_SUITE("const_fixed_interface test");
    test->add(BOOST_TEST_CASE(&const_fixed_interface));
    return test;
}
