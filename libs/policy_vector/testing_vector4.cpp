...
void test_variable_size_policy()
{
    ...
    a.insert(a.begin()+1, 10, -1.0);
    a.insert(a.begin()+11, tmp, tmp+4);
    std::cout << "elems after insert: ";
    for (int i = 0; i< 17; ++i) std::cout << a[i] << ' ';
    std::cout << "..." << std::endl;

    // test erase functions
    a.erase(a.begin());
    a.erase(a.begin(), a.begin()+14);
    std::cout << "elems after insert: ";
...
int main()
{
  ...
  std::cout << "TESTING VARIABLE ALLOCATED_ARRAY WITH INCREMENTAL RESIZING (8)\n";
  test_variable_size_policy< variable_allocated_array<incremental_resize<8> > >();
