...
int test_main( int, char *[] )
{
   ...
   wcscpy( str1, teststr );
   BOOST_CHECK( wcscmp( str1, teststr ) == 0 );
   BOOST_CHECK( wcscmp( str1, teststr ) == wcscmp( teststr, str1 ));
   BOOST_CHECK( wcslen( str1 ) == testlen );

   // copy a different string and validate

   wcscpy( str2, L"12345" );
   BOOST_CHECK( wcscmp( str2, teststr ) != 0 );
   BOOST_CHECK( wcscmp( str2, L"12345" ) == 0 );
   BOOST_CHECK( wcslen( str2 ) == 5 );

   // use the copied string to test wcscat

   wcscat( str2, L"67890" );
   BOOST_CHECK( wcscmp( str2, L"1234567890" ) == 0 );
   BOOST_CHECK( wcslen( str2 ) == 10 );

   // test swprintf

   BOOST_CHECK( swprintf( str3, L"1234%d7890", 56 ) == 10 );
   BOOST_CHECK( wcscmp( str3, L"1234567890" ) == 0 );
   BOOST_CHECK( wcscmp( str2, str3 ) == 0 );
   BOOST_CHECK( wcslen( str3 ) == 10 );

   // test wcscpy with a fixed_string as the 2nd argument

   wcscpy( str3, str1 );
   BOOST_CHECK( wcscmp( str3, teststr ) == 0 );
   BOOST_CHECK( wcslen( str3 ) == testlen );
