...
/**
	...
	Pool where all pointee objects are allocated and tracks memory blocks for later enlisting & marking the @c pack_header the pointee object belongs to.
*/
...
{
public:
    bool init_;										/**< Flag marking initialization of the pointee object to its @c pack_header . */

	intrusive_stack ptrs_;							/**< Stack of all @c pack_ptr s on the heap that will later need to be initlialized to a specific @c pack_header . */
	intrusive_list inits_;							/**< List of all pointee objects that will later need to be initlialized to a specific @c pack_header .*/

    intrusive_list::node pack_tag_;					/**< Tag used to enlist to @c pack_header::elements_ . */
...
template <typename T>
    ...
    public:
        ...
        BOOST_PP_REPEAT_FROM_TO(1, 10, CONSTRUCT_OWNED, pack)
    ...
    public:
		/**
			Cast operator used by @c pack_ptr_common::header() .
		*/
		
        class roofof
        {
            pack * p_;							/**< Address of the @c pack the element belong to. */

        public:
			/**
				Casts from a @c data_type to its parent @c pack object.
				
				@param	p	Address of a @c data_type member object to cast from.
			*/
			
            roofof(data_type * p) : p_(sh::roofof((data_type pack::*)(& pack::elem_), p)) {}
            
			
			/**
				@return		Address of the parent @c pack object.
			*/
			
            operator pack * () const { return p_; }
        };

        
		/**
			Allocates a new @c pack using the pool.
			
			@param	s	Size of the @c pack .
		...
		*/
		...
		/**
			Deallocates a @c pack from the pool.
			
			@param	p	Address of the @c pack to deallocate.
		*/
		
        void operator delete (void * p)
        {
            pool_.deallocate(p, sizeof(pack));
        }
    };


template <>
    class pack<void> : public owned_base
    {
		...
		*/
        ...
        public:
			...
			*/
            ...
            roofof(data_type * p) : p_(sh::roofof((long pack::*)(& pack::elem_), static_cast<long *>(p))) {}
