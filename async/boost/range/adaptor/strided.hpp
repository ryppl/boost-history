...
namespace boost
{
	namespace range_detail
	{
						...
						BaseIterator>
		{
			...
			typedef BOOST_DEDUCED_TYPENAME iterator_adaptor<strided_iterator<BaseIterator>,
				BaseIterator> super_t;
		
		public:
			typedef BOOST_DEDUCED_TYPENAME std::iterator_traits<BaseIterator>::difference_type
					difference_type;
		...
		public:
			...
			strided_range(Difference stride, Rng& r)
				: super_t(make_strided_iterator(begin(r), stride),
					make_strided_iterator(end(r), stride))
			{
			}
		};
	...
	template<class Rng, class Difference>
	inline make_strided_range(Rng& rng, Difference step)
	{
	    return rng | adaptors::stride(step);
	}
	
	template<class Rng, class Difference>
	inline make_strided_range(const Rng& rng, Difference step)
	{
	    return rng | adaptors::stride(step);
	}
} // namespace boost
