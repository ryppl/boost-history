...
typename enable_if<boost::move_detail::is_rv<T>, T &>::type
   forward_constructor(const typename move_detail::identity<T>::type &x)
{
...
typename disable_if<boost::move_detail::is_rv<T>, T &>::type
   forward_constructor(typename move_detail::identity<T>::type &x)
{
   return x;
}
*/
...
#define BOOST_CONSTRUCT_FWD_REF(TYPE)\
   const TYPE & \
//
...
template <class T> inline output_reference forward_constructor(input_reference);
#else
template <class T> inline
T&& forward_constructor (typename move_detail::identity<T>::type&& t)
{  return t;   }
#endif
