...
#ifndef BOOST_SHARED_MESSAGE_QUEUE_HPP
#define BOOST_SHARED_MESSAGE_QUEUE_HPP
...
/*!\file
   Describes inter-process message queue. This class allows sending messages
   between processes and allows blocking non-blockin and timed sending
   and receiving.
*/
   ...
   */
   ...
   public:
    ...
    public:
      ...
      shared_condition           m_cond_recv;
      //Condition block senders when the queue is full
      shared_condition           m_cond_send;
   };

   /*!This is the atomic functor to be executed when creating or opening 
      shared memory. Never throws*/
   struct initialization_func_t
   {
      initialization_func_t(mq_hdr_t *&hdr, 
                            std::size_t maxmsg = 0, 
                            std::size_t maxmsgsize = 0)
         : mp_hdr(hdr), m_maxmsg (maxmsg), m_maxmsgsize(maxmsgsize) {}

      bool operator()(const shared_memory::segment_info_t *info, bool created)
      {
         char      *mptr;

         if(created){
            mptr     = reinterpret_cast<char*>(info->get_user_ptr());
            //Construct the message queue header at the beginning
            BOOST_TRY{
               mp_hdr   = new (mptr) mq_hdr_t(m_maxmsg, m_maxmsgsize);
            }
            BOOST_CATCH(...){
               return false;   
            }
            ...
            //Get the message queue header
            mp_hdr   = reinterpret_cast<mq_hdr_t*>(info->get_user_ptr());
         }
         return true;
      }
      mq_hdr_t        *&mp_hdr;
...
inline shared_message_queue::shared_message_queue()
: mp_hdr(0)
   {}
...
inline std::size_t shared_message_queue::get_mem_size
   (std::size_t max_msg_size, std::size_t max_num_msg)
   {  return mq_hdr_t::get_mem_size(max_msg_size, max_num_msg);   }

inline bool shared_message_queue::create
   (const char *name, std::size_t max_num_msg, std::size_t max_msg_size)
{
   //Get needed shared memory size
   std::size_t memsize = this->get_mem_size(max_msg_size, max_num_msg);

   //Prepare initialization functor
   initialization_func_t func(mp_hdr, max_num_msg, max_msg_size);

   //Create shared memory and execute functor atomically
   if(!m_shmem.create_with_func(name, memsize, func)){
      this->close();
      return false;
   }
   return true;
...
inline bool shared_message_queue::open_or_create
   (const char *name, std::size_t max_num_msg, std::size_t max_msg_size)
{
   //Get needed shared memory size
   std::size_t memsize = this->get_mem_size(max_msg_size, max_num_msg);
   ...
   //Create or open shared memory and execute functor atomically
   if(!m_shmem.open_or_create_with_func(name, memsize, func)){
      this->close();
      return false;
   }
   return true;
...
inline bool shared_message_queue::open
   (const char *name)
{
   //Prepare initialization functor
   initialization_func_t func(mp_hdr);

   //Open shared memory and execute functor atomically
...
inline void shared_message_queue::close()
   {  mp_hdr = 0; m_shmem.close();  }

inline shared_message_queue::result_t 
     shared_message_queue::send(const void *buffer,      std::size_t buffer_size, 
                                unsigned int priority)
   {  return this->send(blocking, buffer, buffer_size, priority); }

inline shared_message_queue::result_t 
     shared_message_queue::try_send(const void *buffer,      std::size_t buffer_size, 
                                    unsigned int priority)
   {  return this->send(non_blocking, buffer, buffer_size, priority); }

inline shared_message_queue::result_t 
     shared_message_queue::timed_send(const void *buffer,      std::size_t buffer_size, 
                                      unsigned int priority,   const xtime &xt)
   {  return this->send(timed, buffer, buffer_size, priority, xt); }

inline shared_message_queue::result_t 
     shared_message_queue::send(block_t block,
                                const void *buffer,      std::size_t buffer_size, 
                                unsigned int priority,   const xtime &xt)
{
   //Check if buffer is smaller than maximum allowed
   if (buffer_size > mp_hdr->m_max_msg_size) {
      return too_big_buffer;
   }

   //---------------------------------------------
   scoped_lock<shared_mutex> lock(mp_hdr->m_mutex);
   //---------------------------------------------
   {
      //If the queue is full execute blocking logic
      if (mp_hdr->m_cur_num_msg >= mp_hdr->m_max_num_msg) {
         ...
         switch(block){
            ...
            case blocking :
               while (mp_hdr->m_cur_num_msg >= mp_hdr->m_max_num_msg){
                  mp_hdr->m_cond_send.wait(lock);
               }
            break;

            case timed :
               while (mp_hdr->m_cur_num_msg >= mp_hdr->m_max_num_msg){
                  if(!mp_hdr->m_cond_send.timed_wait(lock, xt))
                     return timeout;
               }
            break;
            default:
      ...
      msg_hdr_t *free_msg = mp_hdr->free_msg();
      if (free_msg == 0) {
         return internal_error;
      }
      ...
      mp_hdr->queue_free_msg();
      
      //If this message changes the queue empty state, notify it to receivers
      if (mp_hdr->m_cur_num_msg == 1){
         mp_hdr->m_cond_recv.notify_one();
      }
   }  // Lock end
...
inline shared_message_queue::result_t 
   shared_message_queue::receive(void *buffer,              std::size_t buffer_size, 
                                 std::size_t &recvd_size,   unsigned int &priority)
...
inline shared_message_queue::result_t 
   shared_message_queue::timed_receive(void *buffer,              std::size_t buffer_size, 
                                       std::size_t &recvd_size,   unsigned int &priority,
                                       const xtime &xt)
   {  return this->receive(timed, buffer, buffer_size, recvd_size, priority, xt); }

inline shared_message_queue::result_t 
   shared_message_queue::receive(block_t block,
                                 void *buffer,              std::size_t buffer_size, 
                                 std::size_t &recvd_size,   unsigned int &priority,
                                 const xtime &xt)
{
   ...
   //---------------------------------------------
   {
      ...
      if (mp_hdr->m_cur_num_msg == 0) {
         switch(block){
            ...
            case blocking :
               while (mp_hdr->m_cur_num_msg == 0){
                  mp_hdr->m_cond_recv.wait(lock);
               }
            break;

            case timed :
               while (mp_hdr->m_cur_num_msg == 0){
                  if(!mp_hdr->m_cond_recv.timed_wait(lock, xt))
                     return timeout;
               }
            break;
      ...
      msg_hdr_t *top_msg = mp_hdr->top_msg();
      ...
      if (mp_hdr->m_cur_num_msg == (mp_hdr->m_max_num_msg-1)){
         mp_hdr->m_cond_send.notify_one();
      }
   }  //Lock end
...
inline std::size_t shared_message_queue::get_max_num_msg() const
   {  return mp_hdr ? mp_hdr->m_max_msg_size : 0;  }

inline std::size_t shared_message_queue::get_max_msg_size() const
   {  return mp_hdr ? mp_hdr->m_max_msg_size : 0;  }
...
#endif   //#ifndef BOOST_SHARED_MESSAGE_QUEUE_HPP
