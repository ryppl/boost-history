...
namespace boost { namespace langbinding {
   ...
   boost::shared_ptr<inheritance_graph_base> 
   inheritance_graph_base::graph_for_extension(extension_id_t id)
   {
      typedef boost::shared_ptr<inheritance_graph_base> graph_ptr;
      ...
      static graph_map_t graph_map;

      graph_map_t::iterator iter = graph_map.find(id);

      if (iter == graph_map.end())
      {
         graph_ptr p(new inheritance_graph_base);
         graph_map[id] = p;
         return p;
      }
