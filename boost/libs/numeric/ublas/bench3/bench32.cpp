...
struct bench_my_outer_prod {
    ...
    void operator () (int runs, safe_tag) const {
		try {
			static M m (N, N);
            numerics::matrix_range<M> mr (m, 0, N, 0, N);
			static V v1 (N), v2 (N);
            numerics::vector_range<V> vr1 (v1, 0, N), vr2 (v2, 0, N);
			initialize_vector (vr1);
			initialize_vector (vr2);
			numerics::timer t;
			for (int i = 0; i < runs; ++ i) {
                mr = - numerics::outer_prod (vr1, vr2);
//				sink_matrix (mr);
			}
            footer<value_type> () (N * N, N * N, runs, t.elapsed ());
		}
...
//				sink_vector (vr2);
    ...
    void operator () (int runs, fast_tag) const {
		try {
			...
			numerics::timer t;
			for (int i = 0; i < runs; ++ i) {
    			vr2.assign (numerics::prod (mr, vr1));
//				sink_vector (vr2);
			}
            footer<value_type> () (N * N, N * (N - 1), runs, t.elapsed ());
		}
