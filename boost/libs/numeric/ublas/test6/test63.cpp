...
struct test_my_matrix {
    ...
    void operator () (int) const {
		try {
			M m1 (N, N), m2 (N, N), m3 (N, N);
            numerics::symmetric_adaptor<M> sam1 (m1), sam2 (m2), sam3 (m3);
            (*this) (sam1, sam2, sam3);

#ifdef USE_RANGE
            numerics::matrix_range<numerics::symmetric_adaptor<M> > mr1 (sam1, numerics::range (0, N), numerics::range (0, N)), 
																    mr2 (sam2, numerics::range (0, N), numerics::range (0, N)), 
																    mr3 (sam3, numerics::range (0, N), numerics::range (0, N));
            (*this) (mr1, mr2, mr3);
#endif

#ifdef USE_SLICE
            numerics::matrix_slice<numerics::symmetric_adaptor<M> > ms1 (sam1, numerics::range (0, 1, N), numerics::range (0, 1, N)), 
																    ms2 (sam2, numerics::range (0, 1, N), numerics::range (0, 1, N)), 
																    ms3 (sam3, numerics::range (0, 1, N), numerics::range (0, 1, N));
            (*this) (ms1, ms2, ms3);
#endif
		}
        catch (std::exception &e) {
