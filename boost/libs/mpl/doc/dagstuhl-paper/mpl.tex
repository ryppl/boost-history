...
\code{boost::mpl::find} accepts the sequence as a single parameter,
...
sequence built using the \code{boost::mpl::list} template, but
\code{list} is not the only sequence that the library provides you
with. Neither is\code{boost::mpl::find} or any other algorithm
