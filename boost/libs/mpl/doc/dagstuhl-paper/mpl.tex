...
While syntactically simple, however, this
form of \mfn\ does not always interact optimally with the rest of
\Cpp. In particular, the simple \mfn\ form makes it unneccesarily
awkward and tedious to define and work with higher-order \mfns\
(\mfns\ that operate on other \mfns). In order to pass a simple \mfn\
to another template, we need to use \emph{template template
parameters}:
...
This looks different, but it seems to work.\footnote{In fact it's
  already broken: \code{apply\_twice} doesn't even fit the \mfn\
  concept: it requires a template (rather than a type) as its first
...
so we \emph{still} can't embed \code{derive3} in a \code{cons}
...
%% of the lack of so called template typedefs [Sut01]. 
