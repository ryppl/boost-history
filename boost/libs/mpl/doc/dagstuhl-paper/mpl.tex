...
\end{enumerate}
  ...
  \subsubsection{\code{iter\_fold}/\code{iter\_find} as two main 
    iteration algortihms}
  \subsubsection{why iterators}
  \subsubsection{sequences of types, but there are shorcuts for numbers}
...
More importantly (which may be an eye-opening fact for some), type
lists are not the only useful compile-time sequence. Again, the need
for a variety of compile-time containers arises for the same reasons
that we have lists, vectors, deques, and sets in the /Cpp/ standard
library - different containers have different functional and
performance characteristics which determine not only applicability and
efficiency of particular algorithms, but also the expressiveness or
verbosity of the code that uses them.

%% list
%% vector
%% range_c
%% If you can extract something from this:
%% http://groups.yahoo.com/group/boost/message/20770, and this:
%% http://groups.yahoo.com/group/boost/message/20916. Otherwise, may be a
%% paragraph that I just added to the end of tiny_list example will be
%% sufficient.
