...
\subsubsection{The Form of Metafunctions}
...
\Cpp. In order to pass such a \mfn\ to another template, we need to
...
indirection'' applies here. To elevate \mfns\ to the status of
first-class objects, the MPL introduces the concept of a ``\mfn\
class'':\footnote{This form should look familiar to anyone acquainted
with function objects in STL, with the nexted \code{apply} template
taking the same role as the runtime function-call operator}
...
pass the results to a run-time part of the program for furhter 
processing. For example, suppose you've impelemented a complex 
...
is one of the explicit facilites the library provides for run-time 
code generation:
