...
/*
  ...
  Rank List is a C++ STL-style container with fast random access
...
#ifdef RL_DEREF_NULL_ON_ASSERTION_FAILURE
#define RL_ASSERT(C) do { if (!(C)) (*(int*)0)++; } while (0)
#else
#ifdef BOOST_ASSERT
#define RL_ASSERT(C) do { BOOST_ASSERT(C);        } while (0)
#else
#define RL_ASSERT(C) do { assert(C);              } while (0)
#endif
#endif

#ifdef RL_NO_SLOW_ASSERTS
#define RL_ASSERT_HO(C) do {                      } while (0)
#else
#define RL_ASSERT_HO(C) do { RL_ASSERT(C);        } while (0)
#endif

#ifdef RL_ASSERT_BEFORE_THROW
#define RL_ASSERT_EXC(C,E) do { RL_ASSERT (C); \
                                if (!(C)) throw E; } while (0)
#else
#define RL_ASSERT_EXC(C,E) do { if (!(C)) throw E; } while (0)
#endif
...
// (Other headers, containing rank_list methods implementations
  ...
  : private rank_list_node_tree_fields<T,A,W,P>
{
  ...
  public:
    ...
    typedef rank_list<T,A,W,P>                   my_class;
  ...
  public:
    ...
    // See detail/rl_constr_destr.hpp
    //
    ...
    rank_list ();
    rank_list (const my_class & a);
    rank_list (size_type n, const_reference t);
    rank_list (int       n, const_reference t);
    rank_list (long      n, const_reference t);
    rank_list (size_type n);

    template <class IT>
    rank_list (IT from, IT to);

    template <class IT>
    rank_list (IT from, size_type n);

    ~rank_list ();


    // Container assignment
    // See detail/rl_assign.hpp
    //
    ...
    // See detail/rl_random_access.hpp
    //
    ...
    // See detail/rl_sorted_search_tree.hpp
    //
  ...
  friend class rank_list_iterator<T,A,W,P,reference,pointer>;
  friend class rank_list_iterator<T,A,W,P,const_reference,
                                            const_pointer>;

  friend class rank_list_rev_iter<T,A,W,P,reference,pointer>;
  friend class rank_list_rev_iter<T,A,W,P,const_reference,
                                          const_pointer>;
...
#include "detail/rl_alloc.hpp"  // Nodes' allocation/deallocation

#include "detail/rl_build_list.hpp"   // Massive alloc.[+rollback]
#include "detail/rl_build_tree.hpp"   // Build perfectly balanced
#include "detail/rl_constr_destr.hpp" // Constr., destr. and init.
#include "detail/rl_assign.hpp"       // Assignments and cont. swap
#include "detail/rl_compare.hpp"      // Containers comparison

#include "detail/rl_random_access.hpp" // op.[] and iterators +...
#include "detail/rl_begin_end.hpp"     // begin(), end(), r...
#include "detail/rl_front_back.hpp"    // front(), back(), push...

#include "detail/rl_balance.hpp" // Counters update and AVL balance
#include "detail/rl_insert.hpp"  // insert()
#include "detail/rl_erase.hpp"   // erase(), clear()
#include "detail/rl_move.hpp"   // move/splice(), swap(), reverse()
#include "detail/rl_size.hpp"   // size(), max_size(), resize()...

#include "detail/rl_sorted_search_tree.hpp" // sort(),
                                           // [stable_sort()],
                                          // binary_search(),
                                         // insert_sorted(),
                                        // merge(), unique()

#include "detail/rl_npsv.hpp"   // Non Proportional Sequence View
