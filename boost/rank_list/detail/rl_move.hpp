//  boost/rank_list/detail/rl_move.hpp
...
#ifndef _BOOST_RANK_LIST_MOVE_HPP_
#define _BOOST_RANK_LIST_MOVE_HPP_
...
inline
  ...
  (typename rank_list<T,A,W,P>::iterator it1,
   typename rank_list<T,A,W,P>::iterator it2)
{
...
inline
  ...
  rank_list<T,A,W,P>::swap
  (typename rank_list<T,A,W,P>::iterator it1,
   typename rank_list<T,A,W,P>::reverse_iterator it2)
{
  swap_nodes (it1.ptr, it2.ptr);
}
...
inline
  void
  rank_list<T,A,W,P>::swap
  (typename rank_list<T,A,W,P>::reverse_iterator it1,
   typename rank_list<T,A,W,P>::iterator it2)
{
  swap_nodes (it1.ptr, it2.ptr);
}
...
// its position in the sequence of the rlist moves by n
...
// a different rank_list)
//
// Complexity: O(log(M)+log(N))
// (where M and N are the numbers of elements in source
// and destination rlists respectively)

template<class T,class A,class W,class P>
inline
  void
  rank_list<T,A,W,P>::move
  (typename rank_list<T,A,W,P>::iterator src,
   typename rank_list<T,A,W,P>::iterator dst)
{
...
inline
  ...
  rank_list<T,A,W,P>::move
  (typename rank_list<T,A,W,P>::iterator src,
   typename rank_list<T,A,W,P>::reverse_iterator dst)
{
...
inline
  void
  rank_list<T,A,W,P>::move
  (typename rank_list<T,A,W,P>::reverse_iterator src,
   typename rank_list<T,A,W,P>::iterator dst)
{
  move_node (src.ptr, dst.ptr);
}
...
inline
  void
  rank_list<T,A,W,P>::move
  (typename rank_list<T,A,W,P>::iterator src_from,
...
inline
  void
  rank_list<T,A,W,P>::move
  (typename rank_list<T,A,W,P>::reverse_iterator src_from,
   typename rank_list<T,A,W,P>::size_type n,
   typename rank_list<T,A,W,P>::iterator dst)
{
  move_nodes (src_from, n, dst.ptr);
...
inline
  ...
  rank_list<T,A,W,P>::move
  (typename rank_list<T,A,W,P>::iterator src_from,
   typename rank_list<T,A,W,P>::iterator src_to,
   typename rank_list<T,A,W,P>::iterator dst)
{
  difference_type n;
  RL_ASSERT_HO (owner(src_from.ptr)==owner(src_to.ptr));
  n = src_to - src_from;
...
inline
  ...
  rank_list<T,A,W,P>::move
  (typename rank_list<T,A,W,P>::reverse_iterator src_from,
   typename rank_list<T,A,W,P>::reverse_iterator src_to,
   typename rank_list<T,A,W,P>::iterator dst)
{
  difference_type n;
  RL_ASSERT_HO (owner(src_from.ptr)==owner(src_to.ptr));
...
// splice (it/rit,cont): move all contents of another rank_list
// to a given position of this rank_list (group move)
//
// Complexity: see note at the beginnig of this file
...
inline
  void
  rank_list<T,A,W,P>::splice
  (typename rank_list<T,A,W,P>::iterator dst,
   typename rank_list<T,A,W,P>::my_class & src)
{
  RL_ASSERT_HO (owner(dst.ptr)==this);
  RL_ASSERT (&src!=this);
...
// rank_list to a given position of this rank_list (individual
// move)
//
// Complexity: O(log(M)+log(N))
// (where M and N are the numbers of elements in source
...
{
  RL_ASSERT_HO (owner(dst.ptr)==this);
  RL_ASSERT_HO (owner(src_from.ptr)==&src);
...
// rank_list to a given position of this rank_list (range move)
//
// Complexity: see note at the beginnig of this file
   ...
   typename rank_list<T,A,W,P>::iterator src_to)
{
  ...
  RL_ASSERT_HO (owner(dst.ptr)==this);
  RL_ASSERT_HO (owner(src_from.ptr)==&src);
  RL_ASSERT_HO (owner(src_to.ptr)==&src);
...
// reverse(): invert the sequence of the rlist without
...
template<class T,class A,class W,class P>
//not inline
  void rank_list<T,A,W,P>::reverse ()
{
  node_t * p, * next, * tmp;
...
//not inline
  void
  rank_list<T,A,W,P>::swap_nodes
  (typename rank_list<T,A,W,P>::node_t * p,
   typename rank_list<T,A,W,P>::node_t * q)
{
  node_t * tmp, tmpnode;

  RL_ASSERT (p);            // NULL pointer dereference
  RL_ASSERT (q);            // NULL pointer dereference
   ...
   typename rank_list<T,A,W,P>::difference_type n)
{
  ...
  if (!n ||                      // If n==0 or
      (p->m_count==1 &&          // the node to move is
       !p->m_parent->m_parent))  // alone in the rlist,
    return;                      // there's nothing to do

  if (n==1)                      // Just one pos. right?
  {
    RL_ASSERT_EXC (p->m_next->m_parent,
  ...
  {
    RL_ASSERT_EXC (p->m_prev->m_parent,
                   index_out_of_bounds());  // (don't swap end!)
  ...
  RL_ASSERT_EXC (r, index_out_of_bounds());
...
//not inline
  void
  rank_list<T,A,W,P>::move_nodes
  (IT src_from,                                // Source
   typename rank_list<T,A,W,P>::size_type n,   // # nodes to move
   typename rank_list<T,A,W,P>::node_t * dst,  // Destination
