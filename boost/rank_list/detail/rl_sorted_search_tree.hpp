//  boost/rank_list/detail/rl_sorted_search_tree.hpp
...
/*
  detail/rl_sorted_search_tree.hpp
  --------------------------------
...
#ifndef _BOOST_RANK_LIST_SORTED_SEARCH_TREE_HPP_
#define _BOOST_RANK_LIST_SORTED_SEARCH_TREE_HPP_
...
// where it should be) in a sorted rank_list using a
...
// functor)
//
// IMPORTANT: The whole rank_list must be in order
...
inline
  bool
  rank_list<T,A,W,P>::binary_search
  (typename rank_list<T,A,W,P>::const_reference t,
   typename rank_list<T,A,W,P>::const_iterator & it,
   CMP cmp)                                          const
{
...
inline
  ...
  rank_list<T,A,W,P>::binary_search
  (typename rank_list<T,A,W,P>::const_reference t,
   typename rank_list<T,A,W,P>::const_reverse_iterator & it,
   CMP cmp)                                          const
{
#ifdef BOOST_CLASS_REQUIRE
  function_requires<
...
// _exixsting_ order in the rank_list. Two versions of
...
inline
  ...
  rank_list<T,A,W,P>::insert_sorted
  (typename rank_list<T,A,W,P>::const_reference t,
...
// sort(): arrange the elements of the rlist so that the
...
// (where N is the number of elements in the rlist)

template<class T,class A,class W,class P>
template<class CMP>
//not inline
  void rank_list<T,A,W,P>::sort (CMP cmp)
{
#ifdef BOOST_CLASS_REQUIRE
  function_requires<
      BinaryFunctionConcept<CMP,int,const_reference,
...
template<class T,class A,class W,class P>
inline
  void rank_list<T,A,W,P>::sort ()    // Same, but with
{                                     // T::operator<
#ifdef BOOST_CLASS_REQUIRE
  function_requires< LessThanComparableConcept<value_type> >();
#endif
...
// found, place them together in the new rlist but, unlike
...
template<class CMP>
//not inline
  void rank_list<T,A,W,P>::stable_sort (CMP cmp)
{
#ifdef BOOST_CLASS_REQUIRE
  function_requires<
      BinaryFunctionConcept<CMP,int,const_reference,
  ...
  P i;
                            // No P -> no stable sort
  RL_ASSERT (P(0)!=P(1));   // Instantiate the rank_list
                            // specifying std::size_t as P
  if (size()<2)             // parameter instead of the
    return;                 // default value (empty_number)
...
template<class T,class A,class W,class P>
inline
  void rank_list<T,A,W,P>::stable_sort ()  // Same, but with
{                                          // T::operator<
#ifdef BOOST_CLASS_REQUIRE
  function_requires< LessThanComparableConcept<value_type> >();
#endif
...
// merge(): acquire the elements of a donor rlist
// (already in order), and merge them with the elements
// of this rlist (already in order too). Use a 'lesser
// than' comparison functor (an object that has an
// overloaded operator() which recieves two T& and returns
// a bool indicating whether the first T is lesser than the
// second or not) to compare the elements. An additional
...
// comparisons.
//
// IMPORTANT: Both rlists must be in order according to
// the comparison used. Otherwise, the result will have no
// sense.
//
// Complexity: O(M+N)
// (where N is the number of elements in this rlist,
// and M is the number of elements in the donor rlist)

template<class T,class A,class W,class P>
template<class CMP>
//not inline
  void rank_list<T,A,W,P>::merge
  (typename rank_list<T,A,W,P>::my_class & donor,
   CMP cmp)
{
#ifdef BOOST_CLASS_REQUIRE
  function_requires<
...
// unique(): erase duplicates in an already sorted rlist.
...
template<class CMP>
//not inline
  void rank_list<T,A,W,P>::unique (CMP cmp)
{
#ifdef BOOST_CLASS_REQUIRE
  function_requires<
      BinaryFunctionConcept<CMP,int,const_reference,
...
//not inline
  bool
  rank_list<T,A,W,P>::binary_search  // Return true iff found
  (typename rank_list<T,A,W,P>::
