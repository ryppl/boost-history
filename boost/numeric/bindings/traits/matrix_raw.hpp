...
namespace boost { namespace numeric { namespace bindings { namespace traits {
  ...
  template <typename M>
  BOOST_UBLAS_INLINE
  int leading_dimension (const M &m, ublas::row_major_tag) {
    return m.size2();
  }
  template <typename M>
  BOOST_UBLAS_INLINE
  int leading_dimension (const M &m, ublas::column_major_tag) {
    return m.size1();
  }
  template <typename M>
  BOOST_UBLAS_INLINE
  int leading_dimension (const M &m) {
    return bindings::traits::leading_dimension
      (m, BOOST_UBLAS_TYPENAME M::orientation_category());
  }
  template <typename M>
  BOOST_UBLAS_INLINE
  ...
  int matrix_stride1 (const ublas::matrix_slice<M> &m) {
    return m.stride1() * matrix_stride1 (m.data());
  }
  template <typename M>
  BOOST_UBLAS_INLINE
  int matrix_stride2 (const ublas::matrix_slice<M> &m) {
    return m.stride2() * matrix_stride2 (m.data());
  }
  template <typename M>
  BOOST_UBLAS_INLINE
