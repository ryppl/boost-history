...
#ifndef BOOST_INDEXED_SET_MEMBER_HPP
#define BOOST_INDEXED_SET_MEMBER_HPP
 ...
 * of T and "far pointers" to T's. By far pointer to T we mean a type P
  ...
  Type& operator()(const Class& x)const
  {
    return *static_cast<const Type*>(
      static_cast<const void*>(
        static_cast<const char*>(static_cast<const void *>(&x))+OffsetOfMember));
  }
 ...
 * the macro gets into the library, we follow our own heuristics in order to define
 * BOOST_INDEXED_SET_MEMBER as a convenient wrapper of member<> and
 * member_offset<>
 */
...
#define BOOST_INDEXED_SET_MEMBER(Class,Type,MemberName) \
::boost::indexed_sets::member_offset<Class,Type,offsetof(Class,MemberName)>
#else
#define BOOST_INDEXED_SET_MEMBER(Class,Type,MemberName) \
::boost::indexed_sets::member<Class,Type,&Class::MemberName>
#endif
