...
namespace boost { namespace plugin {
    ...
    struct concrete_factory_item<BasePlugin, Concrete, Base, mpl::list<> > 
        : public Base
    {                
        BasePlugin* create(dll_handle dll)
        {
            std::cout << "CF: 0 args\n";
            return new plugin_wrapper<Concrete, mpl::list<> >(dll);
        }
    };

    template<class BasePlugin, class Concrete, class Base, class A1>
    struct concrete_factory_item<BasePlugin, Concrete, Base, mpl::list<A1> > 
        : public Base
    {                
        BasePlugin* create(dll_handle dll, A1 a1)
        {
            std::cout << "CF: 1 args\n";
            return new plugin_wrapper<Concrete, mpl::list<A1> >(dll, a1);
        }
    };

    template<class BasePlugin, class Concrete, class Base, class A1, class A2>
    struct concrete_factory_item<BasePlugin, Concrete, Base, mpl::list<A1, A2> > 
        : public Base
    {                
        BasePlugin* create(dll_handle dll, A1 a1, A2 a2)
        {
            std::cout << "CF: 2 args\n";
            return new plugin_wrapper<Concrete, mpl::list<A1, A2> >(dll, a1, a2);
        }
    };
