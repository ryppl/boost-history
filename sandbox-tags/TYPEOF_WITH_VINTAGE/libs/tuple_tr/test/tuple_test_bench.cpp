...
using boost::tr1::get;
using boost::tr1::tuple_size;
using boost::tr1::tuple_element;
using boost::tr1::tie;
using boost::tr1::make_tuple;
using boost::tr1::ignore;
...
void element_access_test()
{
  ...
  BOOST_STATIC_ASSERT(!(boost::is_const<tuple_element<0, tuple<int, float> >::type>::value));

  // constness should not affect 
  BOOST_STATIC_ASSERT(!(boost::is_const<tuple_element<0, const tuple<int, float> >::type>::value));

  BOOST_STATIC_ASSERT(!(boost::is_const<tuple_element<1, tuple<int, float> >::type>::value));

  // constness should not affect
  BOOST_STATIC_ASSERT(!(boost::is_const<tuple_element<1, const tuple<int, float> >::type>::value));

  // Testing internal element access traits:

  using boost::tr1::tuple_detail::get_result;
  using boost::tr1::tuple_detail::const_get_result;

  BOOST_STATIC_ASSERT((boost::is_same<get_result<0, tuple<int, float> >::type, int&>::value));
  BOOST_STATIC_ASSERT((boost::is_same<const_get_result<0, tuple<int, float> >::type, const int&>::value));

  BOOST_STATIC_ASSERT((boost::is_same<get_result<1, tuple<int, float> >::type, float&>::value));
  BOOST_STATIC_ASSERT((boost::is_same<const_get_result<1, tuple<int, float> >::type, const float&>::value));

  BOOST_STATIC_ASSERT((boost::is_same<get_result<1, tuple<int, const float> >::type, const float&>::value));

  BOOST_STATIC_ASSERT((boost::is_same<get_result<0, tuple<int&, const float&> >::type, int&>::value));
  BOOST_STATIC_ASSERT((boost::is_same<get_result<1, tuple<int&, const float&> >::type, const float&>::value));

  BOOST_STATIC_ASSERT((boost::is_same<const_get_result<0, tuple<int&, const float&> >::type, int&>::value));
  BOOST_STATIC_ASSERT((boost::is_same<const_get_result<1, tuple<int&, const float&> >::type, const float&>::value));
