...
char const literals<char>::mem[] = lit; \
wchar_t const literals<wchar_t>::mem[] = L##lit;
...
char const literals<char>::fmt3[] = "%.*s%.*s";
wchar_t const literals<wchar_t>::fmt3[] = L"%.*ls%.*ls";
...
void do_test_comparison()
{
    ...
    for(size_t n(128); n; --n)
    {
        ...
        BOOST_CHECK(ss1.size() == std::distance(cs1.begin(), cs1.end()));
        BOOST_CHECK(ss1.size() == std::distance(cs1.rbegin(), cs1.rend()));
        ...
        BOOST_CHECK(ss1.size() == std::distance(cs2.begin(), cs2.end()));
        BOOST_CHECK(ss1.size() == std::distance(cs2.rbegin(), cs2.rend()));
        ...
        BOOST_CHECK(std::char_traits<CharT>::length(p1) == std::distance(cs3.begin(), cs3.end()));
        BOOST_CHECK(std::char_traits<CharT>::length(p1) == std::distance(cs3.rbegin(), cs3.rend()));
        ...
        BOOST_CHECK(std::char_traits<CharT>::length(p1) == std::distance(cs4.begin(), cs4.end()));
        BOOST_CHECK(std::char_traits<CharT>::length(p1) == std::distance(cs4.rbegin(), cs4.rend()));
